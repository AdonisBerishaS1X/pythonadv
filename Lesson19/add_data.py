...
if submit_button:
   ...
   book_df.to_csv('bestsellers_with_categories_2022_03_27.csv')
...
col2.metric("Unqiue Books", unique_title)
...
st.subheader("Number of Fictional vs Non-Fictional Books over the Years")
size = book_df.groupby(['Year','Genre']).size().reset_index(name='Counts')
fig=px.bar(size, x='Year', y='Counts', color='Genre', title='Number of Ficitonal vs Non-Ficitonal Books from 2009-2022',
...
top_authors.columns = ['Auhtor', 'Count']
fig = px.bar(top_authors, x='Count', y="Author", orientation = 'h', title='Top 15 Authors by Counts of Books Published (2009-2022)'
             ...
             color='Count', color_continues_scale=px.colors.sequential.Plasma)
st.plotly_chart(fig)

st.subheader("Filter Data by Genre")
grenre_filter = st.selectbox('Selecet Genre', book_df['Genre'].unique())
